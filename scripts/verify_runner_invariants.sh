...
if "${SEARCH_BIN[@]}" "buildSyntheticTextFixture|synthetic library fixture|synthetic.noop" "$ROOT_DIR/conformance_runner.php" >/dev/null; then
...
if ! "${SEARCH_BIN[@]}" "evaluate expression must produce boolean" "$ROOT_DIR/conformance_runner.php" >/dev/null; then
...
if ! "${SEARCH_BIN[@]}" "return 1;" "$ROOT_DIR/src/Cli/Command/GovernanceCommand.php" >/dev/null; then
...
if ! "${SEARCH_BIN[@]}" "check_critical_coverage\\.php" "$ROOT_DIR/composer.json" >/dev/null; then
...
if "${SEARCH_BIN[@]}" "'ops\\.job'" "$ROOT_DIR/conformance_runner.php" >/dev/null; then
